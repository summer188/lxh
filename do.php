...
if ($action == 'import') { //导入XLS
	...
	if (copy($tmp, $file_name)) {
		...
		for ($i=3; $i<=$xls->sheets[0]['numRows']; $i++) {
			...
			$click_url = $xls->sheets[0]['cells'][$i][6];
			...
			$data_values .= "('$sid','$site_logo','$net_logo','$name','$recommend','$click_url','$installment','$has_invoice','$cash_back_rate','$title_attribute','$subject','1','$update_time'),";
		}
		$data_values = substr($data_values,0,-1); //去掉最后一个逗号
		$sql = "insert into lxh_seller_list (sid,site_logo,net_logo,name,recommend,click_url,installment,has_invoice,cash_back_rate,title_attribute,subject,status,update_time) values $data_values";
    ...
    while($row=mysql_fetch_array($result)){
		$sid = iconv('utf-8','gb2312',$row['sid']);
    	...
    	$str .= $sid."\t".$site_logo."\t".$net_logo."\t".$name."\t".$recommend."\t".$click_url."\t".$installment."\t".$has_invoice."\t".$cash_back_rate."\t".$title_attribute."\t".$subject."\t"."\t\n";
    }
