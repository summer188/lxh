 ...
 * Time: 16:36
 */
class PointFiveAction extends PointFourAction{
	//目录列表
	public function five(){
		...
		//搜索
		$where = "period_id=$this->period_id AND level=5";
		if ($keyword!='') {
		...
		foreach($point_list as $key=>&$value){
			...
			$arr = $this->getAllLevelFive($value['alias']);
			//获取其四级目录
	...
	//添加数据
	public function insertFive()
	{
		$post = $_POST;
		//取得其一级目录
		$alias = $post['alias'];
		$alias4 = substr($alias,0,-3);
		...
		if($result){
			$this->success(L('operation_success'), '', '', 'addFive');
		}else{
	...
	public function editFive()
	{
		if(isset($_GET['id']) && intval($_GET['id'])){
			...
			$arr = $this->getAllLevelFive($point_info['alias']);
	...
	public function updateFive(){
		...
		if($result){
			$this->success(L('operation_success'), '', '', 'editFive');
		}else{
			$this->error(L('operation_failure'));
		}
	}
	...
	//excel表格数据保存
	public function insertExcelFive()
	{
		//接收前端传来的post和file
	 ...
	 * @return Array
	 */
	public function getAllLevelFive($alias){
		$arr = array();
