...
{	//显示列表
	public function index()
	{
		...
		$where = '1=1';
		if ($keyword!='') {
			$where .= " AND site_logo LIKE '%".$keyword."%'";
	...
	public function edit()
	{
		...
		foreach ($cate_list as $key=>$value){
			if(in_array($key, $sellect_cate_list)){
				$cate_str.=' <input type="radio" value="'.$key.'" name="cate_id[]" checked="checked"> '.$value.'';
			}else{
				$cate_str.=' <input type="radio" value="'.$key.'" name="cate_id[]"> '.$value.'';
			}
		}
	...
	public function addB2cData(){
		...
		$seller_list=D('seller_list');	  //学科列表
		$seller_cate=D('seller_cate');	  //学科列表
		$seller_list_cate=D('seller_list_cate');	  //学科对应知识点
		$page_size=20;
		...
		if(isset($data[0])){
			foreach ($data as $k=>$v){
				...
				$sort=10;
				$desc=$v['desc'];
				$title_attribute=$v['title_attribute'];
				$subject=$v['subject'];
				$freeshipment=$v['freeshipment'];
				if(is_array($desc)){
					$desc=$name;
				}
				$status=1;
